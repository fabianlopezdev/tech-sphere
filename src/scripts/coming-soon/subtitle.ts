...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    const wordsDataElement: HTMLElement | null = document.getElementById('subtitle-dynamic-words-data')
    if (wordsDataElement && wordsDataElement.textContent) {
      const parsedWords = JSON.parse(wordsDataElement.textContent)
      if (Array.isArray(parsedWords) && parsedWords.length > 0 && parsedWords.every((word: any) => typeof word === 'string')) {
        words = parsedWords
      } else {
  ...
  const flapSpeed: number = 50
  const flapDurationPerLetter: number = 600
  const letterStagger: number = 80
  const wordPause: number = 1500
  // --- End Config ---
